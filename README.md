...
- When the same selected set is loaded again, playback returns to that set's last active video when it came from intentional navigation (playlist click, previous, skip next, mark watched next, or ended auto-next). Initial auto-selection during file loading does not overwrite last-active state. If the exact selected-set record is legacy/stale and a newer recent-active selected file exists, that newer recent-active file can win; otherwise the app starts from the first unwatched item, then the first item.
...
- Progress, watched state, exact selected-set last active video, and per-file recent active signals are saved in the browser's IndexedDB. New last-active records include a source for future restore decisions. File progress is keyed by relative path/name, file size, and last modified timestamp.
